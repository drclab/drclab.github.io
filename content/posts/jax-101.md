...
Because `params` is treated as static (`in_axes=(None,)` via `partial`), `vmap` only lifts over the data axis. This is the same trick we use to compute per-example gradients or to evaluate ensembles in parallel.
