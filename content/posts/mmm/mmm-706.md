...
where wj∗ are population weights and β ∗ are outcome-model parameters. For SDID, we work with a representation that combines unit weighting with additive time adjustments, Y1t (0) ≈
...
Connection to Factor and Imputation Models The factor-model perspective from Chapter 6 provides a unifying lens for these identification assumptions. Suppose untreated potential outcomes follow a factor structure Yit (0) = αi + λt +
