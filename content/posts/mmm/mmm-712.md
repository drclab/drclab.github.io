...
Step 1: Define the Estimand and Cohorts Start by fixing the substantive question and the estimand. For a single treated unit, this is usually an average treatment effect on that unit over a chosen post-treatment window. With several treated units that adopt at the same time, you must decide whether you care about unit-specific effects, an overall ATT, or both. With staggered adoption, define cohorts by adoption time and decide whether you will focus on cohort-specific effects, event-time profiles or calendar-time effects. In staggered designs, the building blocks are cohort-time effects τ (g, t), which can be aggregated into event-time effects θk as described in Section 7.7. Make this explicit before you look at estimates. The aggregation schemes in Section 7.7 show how unitlevel or cohort–time effects map into different summaries. Pre-specifying the target helps prevent you from drifting toward whichever summary looks most flattering ex post.
