...
where ∥a∥2V := a′ Va, η > 0 controls the strength of the penalty, and ∥w∥2 =

2 j wj . Under the simplex

P

constraints, minimising ∥w∥2 discourages putting all the mass on one or two donors and instead favours more diffuse weights. In fact, if w̄ denotes the uniform vector with components 1/N0 , then ∥w− w̄∥2 = ∥w∥2 −1/N0 , so penalising ∥w∥2 is equivalent (up to an additive constant) to shrinking towards uniform weights. In terms P of the variance decomposition in equation (6.24), shrinking ∥w∥2 reduces the Herfindahl index j (wj∗ )2 and P thereby the idiosyncratic-variance component σ 2 (1 + j (wj∗ )2 ). Uniform weights wj = 1/N0 minimise the penalty, while highly concentrated weights incur a large penalty. The parameter η governs the trade-off. If you set η = 0 you recover standard synthetic control. As η grows, the objective gives more weight to simplicity and less to pre-treatment fit, and the solution moves towards uniform weights. The goal is to choose an intermediate η that retains enough structure from the pre-treatment data while damping out idiosyncratic noise. A practical way to do this is to use the same cross-validation logic introduced for ASCM. Split the pre-treatment period into training and validation blocks, estimate ridge
...
Balancing Synthetic Control Ridge SC regularises via the objective function. Balancing SC takes a complementary route and builds explicit covariate balance into the constraints. The optimisation problem becomes min ∥w∥2 w
