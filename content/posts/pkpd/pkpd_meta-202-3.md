...
Written out elementwise, the matrix product computes the covariance of the latent linear predictions that feed each visit average:

$$
\operatorname{Cov}\big(\bar{y}(t_i), \bar{y}(t_j)\big) = \frac{1}{J^{\prime}}\left(\sigma_{\alpha_1}^2 + \sigma_{\alpha_2}^2 x_i x_j\right) + \frac{\sigma_y^2}{J^{\prime}} \mathbb{1}_{\{i = j\}}.
$$

The first term comes from pushing the intercept and slope variances through the polynomial trend; `X_prime*diag_matrix(sigma_a .* sigma_a)*X_prime'` is exactly the matrix form of that expression. The second term is the residual noise contribution, which only touches the diagonal because measurement errors are assumed independent across visits. Dividing the whole expression by `J_prime` reflects that cohort means average the individual trajectories, shrinking both variance components at the familiar $1/J'$ rate.
