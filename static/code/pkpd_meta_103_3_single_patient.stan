...
data {
  int<lower=1> N;                // number of BCVA observations
  vector[N] time;                // observation times (days)
  vector[N] bcva_obs;            // observed BCVA (0-100 letters)
  real start_t;                  // last dosing time (days)
  real lconc0;                   // log concentration at start_t
  real<lower=0> K;               // elimination slope on log scale
  real hill;                     // Hill slope (can be negative)
  real<lower=0> r180;            // long-run potency fraction
  real<lower=0> beta;            // potency decay rate
